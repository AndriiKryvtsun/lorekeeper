...
**Reason**: The campaign/NPC CRUD is migrated from REST route handlers to tRPC
procedures. The `app/api/campaigns/**` route handlers are removed.
**Migration**: Callers use the tRPC `campaignRouter` and `npcRouter` procedures via the
`~/trpc/server` RSC caller (Server Components) or the `~/trpc/react` hooks (Client
Components) instead of HTTP `fetch` to `/api/campaigns/*`. The tRPC HTTP transport lives
at `/api/trpc/*`. (The future `app/api/assistant` streaming handler is unrelated and
remains a route handler.)
