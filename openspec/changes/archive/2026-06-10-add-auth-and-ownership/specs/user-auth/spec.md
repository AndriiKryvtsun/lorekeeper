...
### Requirement: Auth middleware refreshes the session and protects routes
The system SHALL run middleware that refreshes the Supabase session on each request and
protects every application route. Unauthenticated requests to a protected route MUST be
redirected to the login route. Only the public login and auth-callback routes are
exempt.

#### Scenario: Anonymous request to a protected route is redirected
- **WHEN** an unauthenticated user requests a protected route
- **THEN** the middleware redirects them to the login route
...
- **THEN** the middleware refreshes the session cookies before the route runs
