...
Defines the user-facing authentication experience: an `(auth)` route group with
sign-up, sign-in, forgot-password, and reset-password pages where password is the
primary method and magic link is a secondary path. Credential operations run as
Server Actions via the server Supabase client, email confirmation/recovery and PKCE
handlers establish sessions, and the UI is accessible, enumeration-resistant,
CAPTCHA-protected, and free of sensitive logging.
