...
Defines the tRPC v11 API layer for the application: a typed root with a
request context that loads the current user, public and protected procedures,
domain routers merged into an app router, the Next.js App Router HTTP transport,
and the RSC server caller and React Query client used to invoke procedures.
